...
abstract class Model
{
    ...
    {
        global $cfg;
        return json_decode(file_get_contents($cfg["contentFolder"]."/texts.json"), true);
    }
    
    public static function SetText(string $key, string $value) : void
    {
        global $cfg;
        $texts = json_decode(file_get_contents($cfg["contentFolder"]."/texts.json"), true);
        if(isset($texts[$key]))
        {
            $texts[$key]["value"] = $value;
            file_put_contents($cfg["contentFolder"]."/texts.json", json_encode($texts));
        }
        else
        {
